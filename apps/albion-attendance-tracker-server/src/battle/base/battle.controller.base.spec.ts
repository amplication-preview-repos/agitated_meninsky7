...
const service = {
  ...
  battles: () => FIND_MANY_RESULT,
  battle: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("Battle", () => {
  ...
  test("GET /battles/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/battles"}/${nonExistingId}`)
  ...
  test("GET /battles/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/battles"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /battles existing resource", async () => {
